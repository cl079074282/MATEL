...
mt_mat mt_mat_cache::get(const vector<i32>& sizes, i32 depth_channel) {
	basiclog_assert2(!sizes.empty());

	return get((i32)sizes.size(), &sizes[0], depth_channel);
}

mt_mat mt_mat_cache::get(i32 dim, const i32* sizes, i32 depth_channel) {
	...
	mt_mat new_mat;
	new_mat.create_imp(dim, sizes, depth_channel);
...
void mt_mat_cache::statistic_memory() {
	...
	if (total_memory > 1024 * 1024 * 1024 && total_memory > used_memory * 2) {
