...
mt_mat mt_random::random_uniform_iid(i32 rows, i32 cols, i32 depth_channel, f64 min /* = 0.0 */, b8 min_opened /* = sys_true */, f64 max /* = 1.0 */, b8 max_opened /* = sys_true */) {
	i32 sizes[] = {rows, cols};
	return random_uniform_iid(2, sizes, depth_channel, min, min_opened, max, max_opened);
}

mt_mat mt_random::random_uniform_iid(i32 planes, i32 rows, i32 cols, i32 depth_channel, f64 min /* = 0.0 */, b8 min_opened /* = sys_true */, f64 max /* = 1.0 */, b8 max_opened /* = sys_true */) {
	i32 sizes[] = {planes, rows, cols};

	return random_uniform_iid(3, sizes, depth_channel, min, min_opened, max, max_opened);
}

mt_mat mt_random::random_uniform_iid(i32 dims, const i32* sizes, i32 depth_channel, f64 min, b8 min_opened, f64 max, b8 max_opened) {
	...
	for (;;) {
		...
		mt_mat_helper::set_data(ptr_data, depth_channel, random_values, mat.channel());
	}
...
mt_mat mt_random::bernoulli_iid(i32 rows, i32 cols, i32 depth_channel, i32 n, f64 p) {
	i32 sizes[] = {rows, cols};

	return bernoulli_iid(2, sizes, depth_channel, n, p);
}

mt_mat mt_random::bernoulli_iid(i32 dims, const i32* sizes, i32 depth_channel, i32 n, f64 p) {	
	mt_mat mat(dims, sizes, depth_channel);
...
mt_mat mt_random::gaussian_iid(i32 rows, i32 cols, i32 depth_channel, f64 mean, f64 standard_deviation) {
	i32 sizes[] = {rows, cols};

	return gaussian_iid(2, sizes, depth_channel, mean, standard_deviation);
}

mt_mat mt_random::gaussian_iid(i32 planes, i32 rows, i32 cols, i32 depth_channel, f64 mean, f64 standard_deviation) {
	i32 sizes[] = {planes, rows, cols};

	return gaussian_iid(3, sizes, depth_channel, mean, standard_deviation);
}

mt_mat mt_random::gaussian_iid(i32 dims, const i32* sizes, i32 depth_channel, f64 mean, f64 standard_deviation) {
	mt_mat mat(dims, sizes, depth_channel);
...
void mt_random::gaussian_joint(mt_mat& dst, i32 data_type, i32 sample_number, const mt_mat& mean, const mt_mat& covariance) {
	mt_mat eigen_value;
	mt_mat eigen_vector;
