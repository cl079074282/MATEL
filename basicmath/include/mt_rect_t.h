...
public:
	...
	bool operator==(const mt_rect_t<T>& other) const {
		return m_left == other.mLeft && m_top == other.mTop && m_width == other.mWidth && m_height == other.mHeight;
	}
	...
	mt_rect_t(const mt_point_t<T>& leftTop, const mt_size_t<T>& size) {
		setRect(leftTop, size);
	}

	mt_rect_t(const mt_point_t<T>& leftTop, const mt_point_t<T>& rightBottom) {
		setRect(leftTop, rightBottom);
	}

	void setRect(const mt_point_t<T>& leftTop, const mt_size_t<T>& size) {
		m_left = leftTop.x;
		m_top = leftTop.y;
		m_width = size.width;
	...
	void setRect(const mt_point_t<T>& leftTop, const mt_point_t<T>& rightBottom) {
		m_left = leftTop.x;
		m_top = leftTop.y;
		m_width = rightBottom.x - leftTop.x + 1;
		m_height = rightBottom.y - leftTop.y + 1;
	}
	...
	void setTopLeft(const mt_point_t<T>& leftTop) {
		m_top = leftTop.y;
		m_left = leftTop.x;
	}
	...
	bool inBoundary(const mt_point_t<T>& point) const {
	...
	*/
	...
	void adjust_by_append_point(const mt_point_t<T>& point) {
		if (empty()) {
			...
			T minX = min(m_left, point.m_x);
			T minY = min(m_top, point.m_y);
			T maxX = max(right(), point.m_x);
			T maxY = max(bottom(), point.m_y);
			setRect(mt_point_t<T>(minX, minY), mt_point_t<T>(maxX, maxY));
		}
	}
	...
	void adjust_by_append_rect(const mt_rect_t<T>& appendRect) {
		if (empty()) {
			*this = appendRect;
			...
			T minX = min(m_left, appendRect.mLeft);
			T minY = min(m_top, appendRect.mTop);
