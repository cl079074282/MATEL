...
namespace basicmath {
	...
	public:
		...
		static int depth_from_data_type() {
		...
		mt_mat_t(int rows, int cols, int channels) {
			int sizes[] = {rows, cols};
			m_mat = mt_mat(2, sizes, mt_make_depth_channel(depth_from_data_type(), channels));
		}

		mt_mat_t(int planes, int rows, int cols, int channels) {
				int sizes[] = {planes, rows, cols};
				m_mat = mt_mat(3, sizes, mt_make_depth_channel(depth_from_data_type(), channels));
		}

		static mt_mat read(const vector<T>& datas) {
			basiclog_assert2(!datas.empty());

			mt_mat res(1, (basicsys::i32)datas.size(), mt_make_depth_channel(depth_from_data_type(), 1));
