...
#define basiclog_assert(tag, condition)	\
	if (basiclog::log_logger::get_logger() != NULL && basiclog::log_logger::get_logger()->is_enable_error() && !(condition)) basiclog::log_logger::get_logger()->print(L"error", tag, L"", __FILE__, __LINE__)

#define basiclog_assert_message(tag, condition, msg)	\
	if (basiclog::log_logger::get_logger() != NULL && basiclog::log_logger::get_logger()->is_enable_error() && !(condition)) basiclog::log_logger::get_logger()->print(L"error", tag, basicsys::sys_strcombine()<<msg, __FILE__, __LINE__)
